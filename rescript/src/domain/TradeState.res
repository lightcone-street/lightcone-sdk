// TradeState — a rolling, capped trade-history buffer per orderbook, maintained
// from WS `Trades` frames and REST backfills (mirrors rust/src/domain/trade/state.rs
// `TradeHistory`). Newest-first: WS trades carry a monotonic `sequence` and are
// inserted in descending sequence order (duplicated sequences insert before the
// equal entry, matching the Rust `<` probe); REST trades have `sequence == 0`
...
let push = (state: t, trade: Trade.trade): unit =>
  if state.maxSize > 0 {
...
let replace = (state: t, trades: array<Trade.trade>): unit =>
  state.trades = trades->Array.slice(~start=0, ~end=state.maxSize)

// Newest-first view of the buffer.
let trades = (state: t): array<Trade.trade> => state.trades

// The most recent trade, if any.
let latest = (state: t): option<Trade.trade> => state.trades[0]
