// Deposit-token candle series per (asset, resolution) plus the latest per-asset
// price, fed by the WS `DepositPrice` and `DepositAssetPrice` channels.
//
...
type t = {
  mutable candles: Dict.t<array<PriceHistory__Raw.DepositCandle.t>>,
  mutable latest: Dict.t<latestPrice>,
}
...
let getLatestPrice = (state: t, ~depositAsset: Shared.pubkeyStr): option<latestPrice> =>
