"""Constants for the Lightcone program module."""

from solders import system_program as _system_program
from solders import sysvar as _sysvar
from solders.pubkey import Pubkey
from spl.token import constants as _spl_token_constants

# Program IDs
TOKEN_PROGRAM_ID = _spl_token_constants.TOKEN_PROGRAM_ID
ASSOCIATED_TOKEN_PROGRAM_ID = _spl_token_constants.ASSOCIATED_TOKEN_PROGRAM_ID
SYSTEM_PROGRAM_ID = _system_program.ID
RENT_SYSVAR_ID = _sysvar.RENT
INSTRUCTIONS_SYSVAR_ID = _sysvar.INSTRUCTIONS
MPL_TOKEN_METADATA_PROGRAM_ID = Pubkey.from_string(
    "metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s"
)
INITIALIZE_AUTHORITY = Pubkey.from_string(
    "3vYRAzr5X41hrmKMnDCoQJJmPH89S4LLwmFpk8UtwCqr"
)


# Account Discriminators (8 bytes each, SHA-256 hash prefixes)
EXCHANGE_DISCRIMINATOR = bytes([0x1E, 0xC8, 0xDC, 0x95, 0x03, 0x3D, 0x68, 0x32])
MARKET_DISCRIMINATOR = bytes([0xDB, 0xBE, 0xD5, 0x37, 0x00, 0xE3, 0xC6, 0x9A])
ORDER_STATUS_DISCRIMINATOR = bytes([0x2E, 0x5A, 0xF1, 0x49, 0xB2, 0x68, 0x41, 0x03])
USER_NONCE_DISCRIMINATOR = bytes([0xEB, 0x85, 0x01, 0xF3, 0x12, 0x87, 0x58, 0xE0])
POSITION_DISCRIMINATOR = bytes([0xAA, 0xBC, 0x8F, 0xE4, 0x7A, 0x40, 0xF7, 0xD0])
ORDERBOOK_DISCRIMINATOR = bytes([0x2B, 0x22, 0x19, 0x71, 0xC3, 0x45, 0x48, 0x07])
GLOBAL_DEPOSIT_TOKEN_DISCRIMINATOR = bytes(
    [0x25, 0xBE, 0xA1, 0xE8, 0x7B, 0x92, 0x2A, 0x57]
)


# PDA Seeds
SEED_CENTRAL_STATE = b"central_state"
SEED_MARKET = b"market"
SEED_VAULT = b"market_deposit_token_account"
SEED_MINT_AUTHORITY = b"market_mint_authority"
SEED_CONDITIONAL_MINT = b"conditional_mint"
SEED_CONDITION = b"condition"
SEED_ORDER_STATUS = b"order_status"
SEED_USER_NONCE = b"user_nonce"
SEED_POSITION = b"position"
ORDERBOOK_SEED = b"orderbook"
SEED_GLOBAL_DEPOSIT = b"global_deposit"
SEED_FEE_RECEIVER = b"fee_receiver"
SEED_MPL_METADATA = b"metadata"
# Event-authority PDA seed. The program signs its event-batch self-CPI with this
# PDA and requires every public instruction to pass it as a read-only, non-signer
# trailer account immediately before the program account.
SEED_EVENT_AUTHORITY = b"__event_authority"


# Account Sizes (in bytes)
EXCHANGE_SIZE = 216
MARKET_SIZE = 216
ORDER_STATUS_SIZE = 32
USER_NONCE_SIZE = 16
POSITION_SIZE = 80
ORDERBOOK_SIZE = 176
GLOBAL_DEPOSIT_TOKEN_SIZE = 47


# Order Sizes (in bytes)
SIGNED_ORDER_SIZE = 233
ORDER_SIZE = 37
SIGNATURE_SIZE = 64
ORDER_HASH_SIZE = 32

# Backward compatibility aliases
FULL_ORDER_SIZE = SIGNED_ORDER_SIZE
COMPACT_ORDER_SIZE = ORDER_SIZE


# Order Layout Offsets (for FullOrder / SignedOrder)
ORDER_NONCE_OFFSET = 0
ORDER_SALT_OFFSET = 8
ORDER_MAKER_OFFSET = 16
ORDER_MARKET_OFFSET = 48
ORDER_BASE_MINT_OFFSET = 80
ORDER_QUOTE_MINT_OFFSET = 112
ORDER_SIDE_OFFSET = 144
ORDER_MAKER_AMOUNT_OFFSET = 145
ORDER_TAKER_AMOUNT_OFFSET = 153
ORDER_EXPIRATION_OFFSET = 161
ORDER_SIGNATURE_OFFSET = 169


# Instruction Discriminators
INSTRUCTION_INITIALIZE = 0
INSTRUCTION_CREATE_MARKET = 1
INSTRUCTION_ADD_DEPOSIT_MINT = 2
INSTRUCTION_MINT_COMPLETE_SET = 3
INSTRUCTION_MERGE_COMPLETE_SET = 4
INSTRUCTION_CANCEL_ORDER = 5
INSTRUCTION_INCREMENT_NONCE = 6
INSTRUCTION_SETTLE_MARKET = 7
INSTRUCTION_REDEEM_WINNINGS = 8
INSTRUCTION_SET_PAUSED = 9
INSTRUCTION_SET_OPERATOR = 10
INSTRUCTION_WITHDRAW_CONDITIONAL_FROM_POSITION = 11
INSTRUCTION_WITHDRAW_FROM_POSITION = INSTRUCTION_WITHDRAW_CONDITIONAL_FROM_POSITION
INSTRUCTION_ACTIVATE_MARKET = 12
INSTRUCTION_MATCH_ORDERS_MULTI = 13
INSTRUCTION_SET_AUTHORITY = 14
INSTRUCTION_CREATE_ORDERBOOK = 15
INSTRUCTION_WHITELIST_DEPOSIT_TOKEN = 16
INSTRUCTION_DEPOSIT_TO_GLOBAL = 17
INSTRUCTION_GLOBAL_TO_MARKET_DEPOSIT = 18
INSTRUCTION_INIT_POSITION_TOKENS = 19
INSTRUCTION_DEPOSIT_AND_SWAP = 20
INSTRUCTION_WITHDRAW_FROM_GLOBAL = 22
INSTRUCTION_CLOSE_ORDER_STATUS = 24
INSTRUCTION_CLOSE_POSITION_TOKEN_ACCOUNTS = 25
INSTRUCTION_CLOSE_ORDERBOOK = 27
INSTRUCTION_SET_MANAGER = 28
INSTRUCTION_SET_MARKET_FEES = 29
INSTRUCTION_SET_FEE_RECEIVER = 30
INSTRUCTION_CREATE_CONDITIONAL_METADATA = 31
INSTRUCTION_UPDATE_CONDITIONAL_METADATA = 32
INSTRUCTION_SET_ORACLE = 33
INSTRUCTION_ACCEPT_AUTHORITY = 35
INSTRUCTION_ACCEPT_MANAGER = 36
INSTRUCTION_ACCEPT_OPERATOR = 37
INSTRUCTION_SET_DEPOSIT_TOKEN_STATUS = 38
# Reserved discriminator of the program's private event-batch self-CPI. The SDK
# never builds it; it is listed so no public instruction reuses the value.
INSTRUCTION_EVENT_BATCH = 255


# Limits
MAX_OUTCOMES = 6
MIN_OUTCOMES = 2
# Maximum makers in one MatchOrdersMulti or DepositAndSwap instruction.
# This encoding ceiling does not guarantee that a transaction fits runtime limits.
MAX_MAKERS = 11
# Bit 15 selects the taker in a participant mask.
TAKER_MASK = 0x8000
# Encoded participant-mask width in bytes (unsigned 16-bit little-endian).
PARTICIPANT_MASK_LEN = 2
# Maximum deposit mints the program registers per market; add_deposit_mint fails
# with on-chain error 75 (TooManyDepositMints) beyond it.
MAX_DEPOSIT_MINTS_PER_MARKET = 8
# Maximum groups accepted by one init_position_tokens instruction.
# Equal to the per-market cap.
MAX_DEPOSIT_MINTS_PER_IX = MAX_DEPOSIT_MINTS_PER_MARKET
MAX_OUTCOME_NAME_LEN = 32
MAX_OUTCOME_SYMBOL_LEN = 18
MAX_OUTCOME_URI_LEN = 200


# Special Values
NO_WINNING_OUTCOME = 255
