...
test("favorite markets methods use the contracted requests and retry mutations", async () => {
  ...
  const server = createServer((request, response) => {
    ...
    const requestKey = `${request.method} ${request.url} ${request.headers.cookie ?? ""}`;
    const attempt = (attempts.get(requestKey) ?? 0) + 1;
    attempts.set(requestKey, attempt);
    if (request.method !== "GET" && attempt === 1) {
      response.writeHead(503, { "content-type": "application/json" });
      response.end(JSON.stringify({ status: "error", error_details: { reason: "retry" } }));
      return;
    }
  ...
  try {
    ...
    assert.equal((await client.markets().addFavoriteMarket("market/one")).favorited, true);
    assert.equal((await client.markets().addFavoriteMarketWithCookies("market/one", "lightcone-token=test")).favorited, true);
    assert.equal((await client.markets().removeFavoriteMarket("market/one")).favorited, false);
    ...
    assert.deepEqual(requests, [
      ...
      { method: "POST", url: "/api/users/favorite-markets/market%2Fone", cookie: undefined },
      { method: "POST", url: "/api/users/favorite-markets/market%2Fone", cookie: undefined },
      { method: "POST", url: "/api/users/favorite-markets/market%2Fone", cookie: "lightcone-token=test" },
      { method: "POST", url: "/api/users/favorite-markets/market%2Fone", cookie: "lightcone-token=test" },
      { method: "DELETE", url: "/api/users/favorite-markets/market%2Fone", cookie: undefined },
      { method: "DELETE", url: "/api/users/favorite-markets/market%2Fone", cookie: undefined },
