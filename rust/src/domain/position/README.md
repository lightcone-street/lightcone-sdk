...
`deposit_token_balances` returns a required exact nine-decimal `native_sol_balance` alongside the mint-keyed SPL `balances`. Initialize `WalletDepositBalancesState` either with `apply_rest_snapshot(wallet, snapshot)` or a complete WebSocket `Snapshot`, then apply typed events from the matching wallet channel. Existing methods preserve unconditional complete-snapshot replacement even when the lower cross-component slot trails a prior update. Confirmation-aware consumers can call `apply_rest_snapshot_with_minimum_snapshot_slot` or `apply_event_with_minimum_snapshot_slot`; only a complete snapshot below the supplied floor is ignored without mutation, while equal slots and all component/status events retain their normal behavior. Component events are absolute and only apply to initialized matching-wallet state. A matching SPL update with a negative idle balance returns `Rejected` without changing balances or the context slot.
